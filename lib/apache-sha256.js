...
var SHA256 = {
    ...
    encrypt: function(key, salt) {
        ...
        alt_result = crypto.createHash('sha256').update(alt_ctx, encoding).digest('binary');
        ...
        alt_result = crypto.createHash('sha256').update(ctx, encoding).digest('binary');
        ...
        temp_result = crypto.createHash('sha256').update(alt_ctx, encoding).digest('binary');
