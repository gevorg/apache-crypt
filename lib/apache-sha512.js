...
var SHA512 = {
    ...
    encrypt: function(key, salt) {
        ...
        alt_result = crypto.createHash('sha512').update(alt_ctx, encoding).digest('binary');
        ...
        alt_result = crypto.createHash('sha512').update(ctx, encoding).digest('binary');
        ...
        temp_result = crypto.createHash('sha512').update(alt_ctx, encoding).digest('binary');
